...
export default function Post({ post, variant }) {
  ...
  return variant === "variant1" ? (
    <article className="col-span-4  my-4 flex  rounded-lg border-2 bg-[#232428] text-lg">
      <section className=" flex  w-full   flex-col">
        ...
        <div className="text my-2 inline-block p-3">
          <Link to={`${post.id}`} className="">
            <h2 className="text-md  mb-2 inline-block text-[#e4e4e4]">
              {post.title}
            </h2>
            <br />

            <h2 className="inline-block text-sm text-[#d6d0c5]">
              {post.content.split(" ").slice(0, 12).join(" ") + "..."}{" "}
            </h2>
          </Link>
        </div>
        <p className="metadata pointer-events-none inline-block p-2 text-gray-200 no-underline">
